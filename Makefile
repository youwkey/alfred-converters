PACKAGE_NAME := "converters.alfredworkflow"
EXEC_BIN := "alfred-converters"
DIST_DIR := "build"
...
build: mod
	CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -o $(DIST_DIR)/$(EXEC_BIN)-amd64 ./cmd/conv
	CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 go build -o $(DIST_DIR)/$(EXEC_BIN)-arm64 ./cmd/conv
